...
function Table() {
  ...
  return (
    <group>
      ...
      {[[-2.7, -1.7, -1.7], [2.7, -1.7, -1.7], [-2.7, -1.7, 1.7], [2.7, -1.7, 1.7]].map(([x, y, z], i) => (
...
// Tracks THREE.js loader progress and reports to parent
function LoadingTracker({ onProgress, onLoaded }) {
  ...
  useEffect(() => {
    if (!active) {
      // Even if progress not exactly 100, close loader
...
// Inline canvas overlay shown while a Suspense boundary is loading
function CanvasLoader() {
  const { progress } = useProgress();
  return (
    <mesh position={[0, 0, 0]}>
      {/* invisible placeholder — actual UI is outside canvas */}
...
export default function PCScene({ build }) {
  ...
  // Re-trigger loader whenever build changes (user clicks a component)
  useEffect(() => {
    setIsLoading(true);
    setLoadProgress(0);
  }, [build]);
  ...
  return (
    <div style={{
      ...
      background: "radial-gradient(ellipse at 30% 60%, #2a0a4a 0%, #151530 45%, #0b0b18 100%)",
      overflow: "hidden"
    }}>

      {/* Loading overlay — shown on initial load AND every build change */}
      {isLoading && (
      ...
      <div style={{
        ...
        background: "radial-gradient(ellipse at 70% 40%, rgba(120,40,255,0.25) 0%, transparent 55%)",
      }} />

      {/* Neon Fog Layer 2 */}
      <div style={{
        position: "absolute", inset: 0, pointerEvents: "none", zIndex: 0,
        background: "radial-gradient(ellipse at 15% 85%, rgba(0,180,255,0.16) 0%, transparent 50%)",
      }} />

      {/* Horizon glow line */}
      <div style={{
        position: "absolute", bottom: "28%", left: 0, right: 0, height: "1px",
        background: "linear-gradient(90deg, transparent 0%, rgba(140,60,255,0.35) 30%, rgba(0,180,255,0.25) 70%, transparent 100%)",
      ...
      <div style={{
        ...
        background: "radial-gradient(ellipse at 50% 100%, rgba(42,10,74,0.35) 0%, rgba(11,11,24,0.0) 70%)",
        pointerEvents: "none", zIndex: 0,
      }} />
      ...
      {build.case && !isLoading && (
        <div style={{
          ...
          transform: "translateX(-50%)",
          zIndex: 5, display: "flex", gap: "10px",
          ...
          <button
            ...
            onMouseEnter={e => e.currentTarget.style.background = showGlass ? "rgba(14,165,233,0.35)" : "rgba(255,255,255,0.14)"}
            onMouseLeave={e => e.currentTarget.style.background = showGlass ? "rgba(14,165,233,0.25)" : "rgba(255,255,255,0.08)"}
          >
          ...
          <button
            ...
            onMouseEnter={e => e.currentTarget.style.background = rgbEnabled ? "rgba(168,85,247,0.32)" : "rgba(255,255,255,0.14)"}
            onMouseLeave={e => e.currentTarget.style.background = rgbEnabled ? "rgba(168,85,247,0.22)" : "rgba(255,255,255,0.08)"}
          >
      ...
      >
        ...
        {/* Table always loads fast — no suspense needed */}
        <Table />

        {/* Wrap ALL components in a single Suspense — triggers loader for every build change */}
          ...
          <group position={[0, 0.11, 0]} rotation={[0, Math.PI / 4, 0]}>
            {build.case && (
              <>
                ...
                {build?.psu && (
                  <PsuModal isCompatible={compatibilityMap.psu.compatibility !== "bad"} />
                )}
              </>
            )}
            {build.motherboard && (
        ...
        <OrbitControls
          ...
          maxPolarAngle={Math.PI / 2} minPolarAngle={0.3}
        />
      </Canvas>
    </div>
  );
